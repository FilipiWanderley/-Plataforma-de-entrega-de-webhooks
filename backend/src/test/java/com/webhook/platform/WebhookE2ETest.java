...
import com.webhook.platform.application.repository.OutboxEventRepository;
...
class WebhookE2ETest extends AbstractIntegrationTest {
  ...
  void testSuccessfulDelivery_And_Dedupe() throws Exception {
    ...
    stubFor(post(urlEqualTo("/webhook-success")).willReturn(aResponse().withStatus(200)));
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/webhook-endpoints")
    ...
    await()
        ...
        .until(
            () ->
                wireMockServer.findAll(postRequestedFor(urlEqualTo("/webhook-success"))).size()
                    > 0);
    ...
    verify(
        postRequestedFor(urlEqualTo("/webhook-success"))
  ...
  void testRetryOn500() throws Exception {
    ...
    stubFor(post(urlEqualTo("/webhook-error")).willReturn(aResponse().withStatus(500)));
        ...
        .until(
            () ->
                deliveryJobRepository.findAll().stream()
                    .anyMatch(
                        job ->
                            job.getStatus() == DeliveryStatus.PENDING
                                && job.getNextAttemptAt().isAfter(LocalDateTime.now())));
  }
  ...
  void testDlqMaxAttempts() throws Exception {
    ...
    stubFor(post(urlEqualTo("/webhook-dlq")).willReturn(aResponse().withStatus(500)));

    // Create Endpoint with Max Attempts = 2 (Need to update Entity after creation as DTO might not
    // expose it?)
