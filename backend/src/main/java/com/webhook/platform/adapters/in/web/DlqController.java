...
public class DlqController {
  ...
  public ResponseEntity<Void> replay(@PathVariable UUID id) {
    DeliveryJobEntity job =
        jobRepository.findById(id).orElseThrow(() -> new IllegalArgumentException("Job not found"));
    ...
    // Usually replay implies "give it another shot", so we might want to increase max attempts
    // logic or reset count.
