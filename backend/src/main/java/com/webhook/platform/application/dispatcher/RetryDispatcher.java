...
public class RetryDispatcher {
    ...
    public void scheduleRetries() {
        ...
        List<DeliveryJobEntity> jobs = jobRepository.findPendingJobsForUpdateSkipLocked(now, BATCH_SIZE);
        ...
        for (DeliveryJobEntity job : jobs) {
            try {
                ...
                job.setStatus(DeliveryStatus.IN_PROGRESS); // Temporarily mark as IN_PROGRESS until consumer picks up? 
                // Or ENQUEUED if we had that status. Let's use IN_PROGRESS as consumer will execute immediately.
